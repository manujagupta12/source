...
const MARKETS = [
  ...
  { id:"NIFTY",     label:"NIFTY 50 F&O",  icon:"N", color:"#00ff9d",
    strategies:["S1 Calendar Spread","S2 Iron Condor","S3 Short Straddle","S4 0DTE Scalp","S5 Bull Call Spread","S6 Bear Put Spread"] },
  { id:"BANKNIFTY", label:"BANK NIFTY F&O",icon:"B", color:"#f5c518",
    strategies:["S1 Calendar Spread","S2 Iron Condor","S3 Short Straddle","S4 0DTE Scalp","S5 Bull Call Spread","S6 Bear Put Spread"] },
  { id:"FINNIFTY",  label:"FIN NIFTY F&O", icon:"F", color:"#ff6b35",
    strategies:["S1 Calendar Spread","S2 Iron Condor","S5 Bull Call Spread","S6 Bear Put Spread"] },
  { id:"EQUITY",    label:"NSE Equity",    icon:"E", color:"#a78bfa",
    strategies:["E1 EMA Crossover","E2 VWAP Reversion","E3 Opening Range Breakout","E4 Gap Fill","E5 Momentum"] },
];
...
.ticker-inner{display:flex;align-items:center;height:100%;gap:0;white-space:nowrap;animation:ticker 40s linear infinite}
.ticker-inner:hover{animation-play-state:paused}
@keyframes ticker{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
.tick-item{display:inline-flex;align-items:center;gap:8px;padding:0 20px;border-right:1px solid var(--br);height:100%;font-family:var(--mono);font-size:10px;cursor:default}
...
.tick-up{color:var(--grn)}
.tick-dn{color:var(--red)}
.tick-unch{color:var(--muted)}
...
.ws-live{color:var(--grn)}.ws-conn{color:var(--yel)}.ws-err{color:var(--red)}
...
.mkt-name{font-size:11px;font-weight:500;flex:1;line-height:1.3}
...
.feed-ok{color:var(--grn)}.feed-err{color:var(--red)}
...
/* INDEX CARD in market info panel */
.idx-strip{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:18px}
.idx-card{background:var(--s1);border:1px solid var(--br);border-radius:9px;padding:12px 14px;cursor:default;transition:border-color .12s}
...
@keyframes flash-neutral{0%{background:rgba(0,212,255,.2)}100%{background:transparent}}
.flash-up{animation:flash-up .4s ease-out}
.flash-dn{animation:flash-dn .4s ease-out}
.flash-neutral{animation:flash-neutral .4s ease-out}
/* Blink dot for live tick */
...
/* Tick value transition */
.tick-val,.idx-ltp,.eq-ltp{transition:color .15s}
...
.l-btn{width:100%;background:var(--acc);color:#000;border:none;border-radius:7px;padding:10px;font-family:var(--body);font-size:13px;font-weight:700;cursor:pointer;transition:opacity .12s;letter-spacing:.3px;margin-top:3px}
...
function sigClass(dir=""){
  ...
  if(d.includes("SELL")||d.includes("BEAR")||d.includes("SHORT")) return "bear";
  return "neut";
}
...
// ─── INDEX STRIP (Dashboard) ──────────────────────────────────────────────────
function IndexStrip({indices}){
  const main=["NIFTY 50","BANKNIFTY","FINNIFTY","VIX","MIDCAP"];
        ...
        return(
          <div className={`idx-card ${idx._flash||""}`} key={`${i}-${idx._ts||0}`}>
            ...
            <div className="idx-chg" style={{color:col}}>
              ...
              {idx.change!==0?<span style={{color:"var(--muted)",marginLeft:6,fontSize:9}}>
                ({idx.change>0?"+":""}{fmt(idx.change,0)})
              </span>:null}
            </div>
  ...
  return(
    <div className={`sig-card ${sigClass(sig.direction)}`}>
      ...
      <div className="eq-price-hero">
        <div>
          <div className="eq-sym">{sig.symbol}</div>
          {(sig.near_expiry||sig.expiry)?<div style={{fontSize:8,color:"var(--dim)",fontFamily:"var(--mono)",marginTop:2}}></div>:null}
        </div>
        <div style={{textAlign:"right"}}>
          <div className="eq-ltp" style={{color:chg>=0?"var(--grn)":"var(--red)"}}>
            ₹{ltp?fmt(ltp,2):"—"}
          </div>
          <div className={`eq-chg ${chg>=0?"eq-chg-up":"eq-chg-dn"}`}>
            {chg>=0?"+":""}{fmt(chg,2)}%
          </div>
        </div>
      </div>
      ...
      {(sig.high||sig.low)&&<div className="eq-ohlc">
        <span>H: ₹{fmt(sig.high,2)}</span>
        <span>L: ₹{fmt(sig.low,2)}</span>
      ...
      <div className="sig-meta">
        <div className="meta-box">
          <div className="meta-k">Entry</div>
          <div className="meta-v">₹{sig.entry_at?fmt(sig.entry_at,2):"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">Target</div>
          <div className="meta-v" style={{color:"var(--grn)"}}>₹{sig.target_at?fmt(sig.target_at,2):"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">Stop Loss</div>
          <div className="meta-v" style={{color:"var(--red)"}}>₹{sig.sl_at?fmt(sig.sl_at,2):"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">Target ₹</div>
          <div className="meta-v" style={{color:"var(--grn)"}}>+₹{sig.target_pts?fmt(sig.target_pts,0):"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">SL ₹</div>
          <div className="meta-v" style={{color:"var(--red)"}}>-₹{sig.sl_pts?fmt(sig.sl_pts,0):"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">VIX</div>
          <div className="meta-v">{sig.vix||"—"}</div>
        </div>
      </div>
...
function FoCard({sig}){
  ...
  const mkt  = MARKETS.find(m=>m.id===sig.market)||MARKETS[1];
  return(
    <div className={`sig-card ${sigClass(sig.direction)}`}>
      <div className="sig-top">
        <div>
          <div className="sig-strat" style={{color:info.color}}>{sig.strategy}</div>
          <div className="sig-tags">
            <span className="sig-tag" style={{background:mkt.color+"18",color:mkt.color,border:`1px solid ${mkt.color}30`}}>{sig.market}</span>
            <span className="sig-tag" style={{background:info.color+"18",color:info.color,border:`1px solid ${info.color}30`}}>{info.tag}</span>
          </div>
        </div>
        <div className="sig-score-wrap">
      ...
      <div className="fo-strikes">
        <div style={{display:"flex",alignItems:"baseline",gap:4}}>
          <span className="fo-idx">{sig.symbol}</span>
          <span className="fo-spot">
            {sig.spot?`₹${fmt(sig.spot,0)}`:"—"}
            {sig.change_pct!=null&&sig.change_pct!==0&&(
              <span style={{color:chgColor(sig.change_pct),marginLeft:6,fontSize:10}}>
                {sig.change_pct>0?"+":""}{fmt(sig.change_pct,2)}%
              </span>
            )}
          </span>
          {sig.atm&&<span style={{fontSize:9,color:"var(--dim)",marginLeft:8,fontFamily:"var(--mono)"}}>ATM {sig.atm}</span>}
        </div>
        {(sig.near_expiry||sig.far_expiry)&&(
          <div className="fo-exp">
            {sig.near_expiry&&`Near: ${sig.near_expiry}`}
            {sig.far_expiry&&sig.far_expiry!==sig.near_expiry&&`  Far: ${sig.far_expiry}`}
          </div>
        )}
      </div>

      <div className="sig-action">{sig.action}</div>

      <div className="sig-meta">
        <div className="meta-box">
          <div className="meta-k">Spread</div>
          <div className="meta-v">{sig.entry_spread?fmt(sig.entry_spread,1)+"pts":"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">Target</div>
          <div className="meta-v" style={{color:"var(--grn)"}}>{sig.target_pts?"+"+sig.target_pts+"pts":"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">Stop Loss</div>
          <div className="meta-v" style={{color:"var(--red)"}}>{sig.sl_pts?"-"+sig.sl_pts+"pts":"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">Lot Size</div>
          <div className="meta-v">{sig.lot_size||"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">Margin</div>
          <div className="meta-v">{sig.margin_req?`₹${Math.round(sig.margin_req/1000)}k`:"—"}</div>
        </div>
        <div className="meta-box">
          <div className="meta-k">VIX</div>
          <div className="meta-v">{sig.vix||"—"}</div>
      ...
      {sig.reason&&<div className="sig-reason">{sig.reason}</div>}
      <div className="sig-foot">
        <div className="sig-src">📡 {sig.source||"NSE+MultiTrade"}</div>
        <span className={`risk-badge r${(sig.risk||"M")[0]}`}>{sig.risk||"MEDIUM"}</span>
      </div>
    </div>
  );
...
function SigCard({sig}){
  return sig.market==="EQUITY"?<EqCard sig={sig}/>:<FoCard sig={sig}/>;
}
...
function SignalsTab({signals,market,strategy,indices}){
  const filtered=signals.filter(s=>{
    if(market!=="ALL"&&s.market?.toUpperCase()!==market) return false;
    if(strategy&&!s.strategy?.toUpperCase().includes(strategy.split(" ")[0].toUpperCase())) return false;
    return true;
  });
  return(
    <div>
      ...
      {!filtered.length?(
        <div className="empty">
          ...
          <div className="empty-s">{market!=="ALL"?`${market} signals appear during market hours (9:15–15:30)`:"Waiting for NSE data — signals refresh every 30 seconds"}</div>
        </div>
      ):(
        <div className="sigs-grid">
          {filtered.map((s,i)=><SigCard key={s.id||i} sig={s}/>)}
        </div>
      )}
    </div>
  );
...
function AnalyticsTab(){
  ...
  const chart=Object.entries(byS).map(([k,v])=>({
    name:k.replace(/^S\d\s/,"").replace(/^E\d\s/,"").slice(0,12),
    pnl:Math.round(v.total_pnl||0),wr:v.win_rate||0
  }));
  return(
    <div>
      ...
      {!chart.length&&(
        <div className="empty"><div className="empty-ico">📈</div>
          ...
          <div className="empty-s">Log and close trades to see analytics</div>
        </div>
      )}
    </div>
  );
...
function TradesTab(){
  ...
  const open=tr.open_trades||[]; const closed=tr.closed_trades||[];
  return(
    <div>
      <div className="stats-grid">
        {[
          ...
          {l:"Closed",       v:tr.closed_count||closed.length, c:"var(--acc)"},
        ].map((s,i)=>(
          <div key={i} className="stat-card">
            <div className="stat-lbl">{s.l}</div>
            <div className="stat-val" style={{color:s.c,fontSize:18}}>{s.v}</div>
      ...
      {open.length>0&&(
        <div className="card" style={{marginBottom:16,overflowX:"auto"}}>
          <div className="card-lbl" style={{marginBottom:10}}>Open Positions</div>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
            <thead><tr style={{color:"var(--muted)",borderBottom:"1px solid var(--br)"}}>
              {["ID","Strategy","Market","Instrument","Lots","Entry","Status"].map(h=>(
                <th key={h} style={{textAlign:"left",padding:"6px 8px",fontWeight:500,fontSize:10}}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {open.map((t,i)=>(
                <tr key={i} style={{borderBottom:"1px solid rgba(27,48,80,.5)"}}>
                  <td style={{padding:"8px",fontFamily:"var(--mono)",fontSize:10}}>{t.id}</td>
                  <td style={{padding:"8px",fontSize:10}}>{t.strategy}</td>
                  <td style={{padding:"8px"}}>
                    <span style={{fontSize:8,padding:"2px 6px",background:"rgba(0,212,255,.08)",color:"var(--acc)",borderRadius:3,fontFamily:"var(--mono)"}}>{t.market?.toUpperCase()}</span>
                  </td>
                  <td style={{padding:"8px",fontFamily:"var(--mono)",fontSize:10}}>{t.instrument}</td>
                  <td style={{padding:"8px"}}>{t.lots}</td>
                  <td style={{padding:"8px",fontFamily:"var(--mono)"}}>{t.entry_spread}</td>
                  <td style={{padding:"8px"}}>
                    <span style={{fontSize:8,padding:"2px 8px",background:"rgba(245,197,24,.08)",color:"var(--yel)",border:"1px solid rgba(245,197,24,.2)",borderRadius:3}}>OPEN</span>
                  </td>
                </tr>
              ))}
            </tbody>
...
export default function App(){
  ...
  // Clock
  useEffect(()=>{const t=setInterval(()=>setClock(new Date()),100);return()=>clearInterval(t);},[]);  // 100ms clock
  ...
  const IST_MS=("00"+clock.getMilliseconds()).slice(-3).slice(0,1); // tenths only
  ...
  const bear=signals.filter(s=>["SELL","BEAR","SHORT"].some(k=>s.direction?.toUpperCase().includes(k))).length;
        ...
        <div className="main">
          ...
          <header className="topbar">
            ...
            <div className="topbar-right">
              ...
              {regime?.nifty_ltp>0&&<div className="badge">NIFTY ₹{fmt(regime.nifty_ltp,0)}</div>}
              {regime?.nifty_chg!=null&&<div className="badge" style={{color:regime.nifty_chg>=0?"var(--grn)":"var(--red)"}}>
                {regime.nifty_chg>=0?"+":""}{fmt(regime.nifty_chg,2)}%
              </div>}
              <div className={`badge ws-${wsStatus==="live"?"live":wsStatus==="connecting"?"conn":"err"}`}
                style={{display:"flex",alignItems:"center",gap:5}}>
                {wsStatus==="live"?<><span style={{width:6,height:6,borderRadius:"50%",background:"var(--grn)",display:"inline-block",animation:"pulse 1s infinite"}}/>LIVE 1s</> 
              ...
              <>
                <div className="stats-grid" style={{marginBottom:16}}>
                  {[
                    ...
                    {l:"Market Bias",     v:regime?.bias||"—",                 c:regime?.nifty_chg>0?"var(--grn)":regime?.nifty_chg<0?"var(--red)":"var(--yel)"},
                  ].map((s,i)=>(
                    <div key={i} className="stat-card">
                      <div className="stat-lbl">{s.l}</div>
                      <div className="stat-val" style={{color:s.c,fontSize:i===3?13:20}}>{s.v}</div>
