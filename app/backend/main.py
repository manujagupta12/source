...
v3.0.0 — PCR strategy + paper trading endpoints + subscription + dashboard fixes
"""
...
try:
    ...
    _pcr_fetcher  = NseOiFetcher()
    _pcr_strategy = PCRStrategy(_pcr_fetcher)
    _PCR_OK = True
...
_MOCK_STRATEGIES = [
    ...
    "S4 MOMENTUM", "S5 PCR CONTRARIAN", "S7 RATIO SPREAD"
]
...
def _mock_signal():
    ...
    # PCR mock fields
    pcr_val     = round(random.uniform(0.55, 1.45), 3)
    return {
...
def _pcr_signal_live(vix=None):
    """Generate live PCR signals — runs every ~3 minutes to respect NSE rate limits."""
    if not _PCR_OK or not _pcr_strategy:
        return []
    try:
        return _pcr_strategy.generate_all(vix=vix)
    except Exception as e:
        logging.warning(f"[PCR] Live fetch error: {e}")
        return []
...
def _fetch_live_indices() -> list:
    ...
    if not indices:
        for label, base in [("NIFTY",24500),("BANKNIFTY",53000),("FINNIFTY",23000),
                             ("VIX",14.5),("MIDCAP",52000),("IT",37000)]:
            indices.append({
                "label": label, "ltp": round(base+random.uniform(-base*0.008,base*0.008),2),
                "change_pct": round(random.uniform(-1.5,1.5),2),
                "change": round(random.uniform(-200,200),2),
                "high": round(base*1.01,2), "low": round(base*0.99,2), "_ts": int(time.time()),
            })
    return indices
...
async def signal_loop():
    ...
    while True:
        ...
        # PCR strategy runs every ~3 minutes (cycle 36 = 36×5s = 180s)
        if cycle % 36 == 0:
            vix = None
            try:
                vix_idx = next((i for i in _latest_indices if i["label"]=="VIX"), None)
                vix = vix_idx["ltp"] if vix_idx else None
            except Exception: pass
            pcr_sigs = await asyncio.get_event_loop().run_in_executor(None, _pcr_signal_live, vix)
            for ps in pcr_sigs:
                _db["signals"].append(ps)
                await broadcaster.broadcast({"type":"signal","data":ps})
            if pcr_sigs:
                _db["signals"] = _db["signals"][-300:]
                logging.info(f"[PCR] Broadcast {len(pcr_sigs)} PCR signal(s)")
...
async def lifespan(app: FastAPI):
    ...
    logging.info(f"  AlgoTrade API v3  XLS:{_XLS_OK}  NSE:{_NSE_OK}  PCR:{_PCR_OK}")
    task = asyncio.create_task(signal_loop())
    yield
    task.cancel()

app = FastAPI(title="AlgoTrade API", version="3.0.0", lifespan=lifespan)
...
async def fo_ingest(req: FoSignalIngest):
    signal={
        ...
        "near_strike":req.near_strike,"far_strike":req.far_strike,"spread":req.spread,
        "fair_value":req.fair_value,"deviation":req.deviation,"vix":req.vix,
        "regime":req.regime or "LIVE","risk":req.risk or "MEDIUM",
...
def pcr_signals_endpoint(symbol: str="NIFTY"):
    """On-demand PCR signal for a symbol."""
    if not _PCR_OK:
        return {"signals":[],"message":"PCR module not loaded"}
    vix=None
    try:
        vix_idx=next((i for i in _latest_indices if i["label"]=="VIX"),None)
        vix=vix_idx["ltp"] if vix_idx else None
...
def trades_shorthand(user=Depends(get_current_user)):
    ...
    closed_c=sum(1 for t in trades if t["status"]=="CLOSED")
    pnl=sum(t.get("pnl_inr") or 0 for t in trades if t["status"]=="CLOSED")
    return {"trades":trades[-100:],"total":len(trades),
            "open_count":open_c,"closed_count":closed_c,"total_pnl":pnl}
...
def _get_paper(email):
    if email not in _paper:
        _paper[email]={
            "balance":PAPER_INITIAL,"initial":PAPER_INITIAL,
...
def paper_account(user=Depends(get_current_user)):
    ...
    realised=sum(t.get("pnl_inr") or 0 for t in closed_t)
    return {
        **acc,"total_pnl":pnl,"pnl_pct":round(pnl/acc["initial"]*100,2),
...
def leaderboard():
    ...
    for email,acc in _paper.items():
        pnl=acc["balance"]-acc["initial"]; user=_db["users"].get(email,{})
...
def subscription_status(user=Depends(get_current_user)):
    ...
    return {"plan":plan,"tier":tier,"plan_info":plan_info,
...
def analytics_pnl(user=Depends(get_current_user)):
    ...
    return {
        "pnl":sorted(by_date.values(),key=lambda x:x["date"]),
...
@app.get("/health")
def health_check():
    return {"status":"ok","version":"3.0.0","users":len(_db["users"]),
...
@app.get("/")
def root():
    return {"message":"AlgoTrade API v3.0","docs":"/docs","health":"/health","ws":"/ws/signals"}
